...
// Updated interface following StrippyPage
interface StrippyPage {
  id_strippy: string | number;
  name_strippy: string;
  dis_strippy?: string;
  filename?: string;
  tol_page?: number;
...
interface Props {
  relatedStrippies: StrippyPage[];
  itemsPerPage?: number;
}

const RelatedStrippiesWithPagination: React.FC<Props> = ({
  relatedStrippies,
  itemsPerPage = 24,
}) => {
  const [loading, setLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const [strippyData, setStrippyData] = useState<StrippyPage[]>([]);

  // Simulate async fetch on mount
  useEffect(() => {
    setLoading(true);
    const timer = setTimeout(() => {
      setStrippyData(relatedStrippies); // Simulate loading delay (1s)
      setLoading(false);
    }, 1000);
  ...
  const totalPages = Math.ceil(strippyData.length / itemsPerPage);

  // Slice data for current page
  const pagedStrippies = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return strippyData.slice(start, start + itemsPerPage);
  }, [strippyData, currentPage, itemsPerPage]);
        ...
        <>
          <ScrollArea className="h-[500px] md:h-[700px] lg:h-[900px] pr-2 rounded-lg">
            <div className="grid gap-4 grid-cols-3 md:grid-cols-5 lg:grid-cols-7 justify-items-center">
              {pagedStrippies.map((strippy) => (
                <a
                  key={strippy.id_strippy}
                  href={`/strippy/${strippy.id_strippy}`}
                  className="group flex flex-col items-center w-full cursor-pointer select-none"
                >
                  {strippy.dis_strippy && (
                    <Image
                      src={strippy.dis_strippy}
                      alt={strippy.name_strippy}
                      width={100}
                      height={100}
...
export default RelatedStrippiesWithPagination;
